...
def evaluate_model(total_errors,X,y,X_train,y_train,y_test,X_test, i):
...
def perform_k_fold(X,y,folds):
    ...
    for i in range(folds):
        ...
        if i == 1:
            ...
            total_errors = evaluate_model(total_errors ,X,y, X_train, y_train, y_test, X_test,i)
...
def main():
    ...
    error = perform_k_fold(X, y, 5)

    error_LSTM = np.mean(error)
    print("LSTM Neural Network RMSE: " + str(error_LSTM))
    return error_LSTM
